@section('scripts')
	...
	function detalleProject(){
		if (activeDProject){
			...
			//$('.complete').removeClass('active').animate({opacity: 1}, 600, function() {});
			$('.complete-1').removeClass('active');
		}else{
			var rutaImg='{{asset("images/close-gray.png")}}';
			$('.vista-project-1').html('CERRAR<img src="'+rutaImg+'" alt="X">');
			activeDProject = true;			
			//$('.complete').addClass('active').animate({opacity: 1}, 600, function() {});
			$('.complete-1').addClass('active');
		}
	};
	</script>
	<script type="text/javascript">
	$(document).ready( function() {
	    ...
	    //Vista button
		$('body').on('click', '.vista-gray', function(e){
			e.preventDefault();
			modalEditorial();
		...
		//Vista detalle
		$('body').on('click', '.detalle-editorial img', function(e){
			e.preventDefault();
			$('.detalle-editorial .detalle').toggleClass('active');
		});

		//Vista detalle
		$('body').on('click', '.vista-project, .ver-mas', function(e){
			e.preventDefault();
			vistaProject();
		...
		//Vista detalle
		$('body').on('click', '.vista-project-1, .ver-mas', function(e){
			e.preventDefault();
			detalleProject();
			//$('.complete').toggleClass('active');
		})
					...
					if( data.success == true )
					{
						...
						if( data.proyecto.imagenes.length > 1 )
						{
							...
							$.each(data.proyecto.imagenes, function(index, value){
								...
								var cls = '';
								if( activeDProject == true ){
									cls = 'active';
								}
								
								salida += '<div class="img-cont complete-1 '+cls+'">';
    ...
    <script type="text/javascript">
	$('.preparacion-slides').cycle({ fx: 'fade', width: 'auto', height:'100%', containerResize: 0, slideResize:0, fit:1, nowrap:0, random:false, timeout:2000 });	
	$('.incubacion-slides').cycle({ fx: 'fade', width: 'auto', height:'100%', containerResize: 0, slideResize:0, fit:1, nowrap:0, random: false, timeout:2000 });
	$('.iluminacion-slides').cycle({ fx: 'fade', width: 'auto', height:'100%', containerResize: 0, slideResize:0, fit:1, nowrap:0, random: false, timeout:2000 });
	$('.verificacion-slides').cycle({ fx: 'fade', width: 'auto', height:'100%', containerResize: 0, slideResize:0, fit:1, nowrap:0, random: false, timeout:2000 });
    </script>
